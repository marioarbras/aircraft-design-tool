...
for i = 1 : length(vehicle.components)
    if is_type(vehicle.components{i}, 'fuselage')
        for j = 1 : length(mission.segments)
            ...
            vehicle.components{i}.segments{comp_segment_id}.base_drag_coefficient = friction_coeff(vehicle.components{i}.length, mean(mission.segments{j}.velocity), mean(mission.segments{j}.speed_sound), mean(mission.segments{j}.density), air_viscosity(mean(mission.segments{j}.temperature))) *...
    ...
    elseif is_type(vehicle.components{i}, 'wing')
        for j = 1 : length(mission.segments)
            ...
            vehicle.components{i}.segments{comp_segment_id}.base_drag_coefficient = friction_coeff(vehicle.components{i}.mean_chord, mean(mission.segments{j}.velocity), mean(mission.segments{j}.speed_sound), mean(mission.segments{j}.density), air_viscosity(mean(mission.segments{j}.temperature))) *...
